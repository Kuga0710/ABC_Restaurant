...
const StaffList = () => {
    ...
    useEffect(() => {
        ...
        const downloadCsv = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/user/users/csv?role=staff', {
                    responseType: 'blob', // Important for file downloads
                });
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'staff_report.csv'); // Filename for the downloaded file
